...
function StatCard({ label, value, sub, valueClass = "" }) {
  return (
    <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 transition-colors duration-200">
      ...
      <p className={`text-2xl font-medium text-slate-900 dark:text-white ${valueClass}`}>{value}</p>
...
function Skeleton() {
  return (
    <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 transition-colors duration-200">
      <div className="mb-2.5 h-3 w-1/2 animate-pulse rounded bg-slate-100 dark:bg-slate-700" />
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      ...
      <StatCard
        ...
        valueClass={hasClaimed ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}
      />
      {!isActive && (
        <div className="sm:col-span-2 rounded-md bg-amber-50 dark:bg-amber-900/30 px-3 py-2 text-sm text-amber-700 dark:text-amber-400">
